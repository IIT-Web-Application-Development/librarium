...
var createTableRowsForindex = function(index,cursor){
	...
	var td7 = $("<td id='booksAvailable"+index+"'>"+cursor.value.availableBooks+"</td>"); 
	var td8 = $("<td id='checkBox"+index+"'><a id="+sNo+" class='review' href='#'>review</a></td>"); 
	var td9 = $("<td id='booksAvailable"+index+"'><input type='checkbox' class='form-check-input radioForCheckout'></td>"); 
	var trClose = $("</tr>");
	$("#mainCategorizedTbody").append(trOpen.append(td1).append(td2).append(td3).append(td4).append(td5).append(td6).append(td7).append(td8).append(td9).append(trClose));
}
...
var updateCheckoutStatus = function(checkedBook){
	...
	request.onsuccess = function(e) {
		...
		objectStore.openCursor().onsuccess = function(event) {
	 		var cursor = event.target.result;
	 		if(cursor){
	 			if(cursor.value.bookName == checkedBook){
					cursor.value.checkout = true;
		...
		objectStore.openCursor().onsuccess = function(event) {
   		 	...
   		 	if(cursor){
   		 		if(category === cursor.value.category){
   		 			createTableRowsForindex(currentThread,cursor);
